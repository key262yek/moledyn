...
pub struct ContPassiveInteractSearcher{            // 연속한 시스템에서 Passive하게 움직이는 mergeable searcher
...
impl ContPassiveInteractSearcher{
    ...
    pub fn new_uniform(sys : &dyn SystemCore<f64>, target : &dyn TargetCore<f64>,
        ...
        Ok(ContPassiveInteractSearcher{
            searcher_type : SearcherType::ContinuousPassiveInteracting,
            int_type: int_type,
            mtype   : mtype,
...
impl_argument_trait!(ContPassiveInteractSearcher, "Searcher", ContPassiveInteractSearcherArguments, 5,
    searcher_type, SearcherType, SearcherType::ContinuousPassiveInteracting;
    int_type, InteractType, "Type of Interaction. ex) Exponential(dim, gamma, strength), Coulomb(dim, strength)",
...
impl ContPassiveInteractSearcher{
    #[allow(dead_code)]
    pub fn convert_from(argument : &ContPassiveInteractSearcherArguments) -> Vec<Self>{
        let dim : usize;
...
impl SearcherCore<f64> for ContPassiveInteractSearcher{
    fn pos(&self) -> &Position<f64>{
        &self.pos
    }
}

impl Passive<f64, f64> for ContPassiveInteractSearcher{
...
impl Interaction<f64, f64> for ContPassiveInteractSearcher{
    // Mutual displacement
...
mod tests{
    ...
    fn test_new(){
        ...
        let searcher1 = ContPassiveInteractSearcher::new(InteractType::Exponential(2, 1f64, 0f64),
            MoveType::Brownian(1f64), pos.clone());
        assert_eq!(searcher1, ContPassiveInteractSearcher{
            searcher_type : SearcherType::ContinuousPassiveInteracting,
    ...
    fn test_uniform() -> Result<(), Error>{
        ...
        let searcher1 = ContPassiveInteractSearcher::new_uniform(&system, &target, &mut rng1,
    ...
    fn test_interaction_trait() -> Result<(), Error>{
        ...
        let mut searcher1 = ContPassiveInteractSearcher::new(int_type, mtype, Position::<f64>::new(vec![0.0, 0.0]));
        let searcher2 = ContPassiveInteractSearcher::new(int_type, mtype, Position::<f64>::new(vec![2.5, 0.0]));
