...
pub struct ContPassiveLJAgent{            // 연속한 시스템에서 Passive하게 움직이는 mergeable agent
    ...
    pub dim     : usize,                        // Dimension of system contains agent
    pub pos     : Position<f64>,                // position of agent
    pub ptl_size: f64,                          // particle size of agent, should be a order of 1
...
impl_argument_trait!(ContPassiveLJAgent, "Agent", ContPassiveLJAgentArguments, 5,
    ...
    ptl_size, f64,          "Particle size of agent. Should be an order of 1 ex) 1.0, 2.0",
    strength, f64,          "Strength of interaction",
    num_agent, usize,    "Number of Agent");

impl ContPassiveLJAgent{
    #[allow(dead_code)]
    pub fn convert_from(argument : &ContPassiveLJAgentArguments) -> Vec<Self>{
        let dim : usize;
...
impl AgentCore<f64> for ContPassiveLJAgent{
...
impl Passive<f64, f64> for ContPassiveLJAgent{
...
impl Interaction<f64, f64> for ContPassiveLJAgent{
...
mod tests{
    ...
    fn test_new(){
        ...
        let agent1 = ContPassiveLJAgent::new(InteractType::LennardJones(1f64),
            MoveType::Brownian(1f64), pos.clone(), 0f64);
        assert_eq!(agent1, ContPassiveLJAgent{
            agent_type : AgentType::ContinuousPassiveInteracting,
    ...
    fn test_invalid_argument(){
        ...
        let _agent1 = ContPassiveLJAgent::new(InteractType::Exponential(1, 2f64),
            MoveType::Brownian(1f64), pos.clone(), 0f64);
    }
    ...
    fn test_uniform() -> Result<(), Error>{
        ...
        let agent1 = ContPassiveLJAgent::new_uniform(&system, &target, &mut rng1,
    ...
    fn test_interaction_trait() -> Result<(), Error>{
        ...
        let mut agent1 = ContPassiveLJAgent::new(int_type, mtype, Position::<f64>::new(vec![0.0, 0.0]), 0f64);
        let agent2 = ContPassiveLJAgent::new(int_type, mtype, Position::<f64>::new(vec![2.5, 0.0]), 0f64);
